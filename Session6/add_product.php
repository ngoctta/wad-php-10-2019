    ...
    if(isset($_POST['add_product'])) {
      ...
      if(count($error) == 0){
        ...
        if ($image_file['error'] == 0) {
          ...
          $file_arr = explode('.', $image_file['name']);
         
          $file_arr[count($file_arr) - 2 ] .= strtotime(date('Y-m-d'));

          $image = implode('.', $file_arr);
            
          move_uploaded_file($image_file['tmp_name'], 'upload/'.$image);
        }
        $sql = "INSERT INTO products(title, image, description, price, discount, product_category_id) VALUES ('$title', '$image', '$description', '$price', '$discount', '$category')";
